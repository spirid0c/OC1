...
// --- MACHINE Ã€ Ã‰TATS (ARCADE) ---
let gameState = 'SELECT'; // 'SELECT' (PrÃ©paration) ou 'SIMULATE' (Action)
let currentSlotIndex = 0;
let hoveredCityIndex = 0;
let pedalPressTime = 0;
let pedalHoldTimer = null;
let targetCameraSpherical = null; // Pour faire voler la camÃ©ra

let selectedSlots = [-1, -1, -1, -1, -1]; // Vide au dÃ©part
const TRANSLATIONS = {
    EN: {
        archives: "Archives",
        localImport: "Local Import",
        backArchives: "â† Back to Archives",
        dragFiles: "Drag your files here",
        browse: "Browse",
        tracer: "Tracer",
        simulationDay: "Simulation Day",
        switchTracer: "Switch Tracer (Japan â†” Brazil)",
        ...
        legendVapor: "Atmospheric Vapor Tracer [kg/mÂ²]",
        legendRain: "Tracer rain > 0.1 mm/day",
        months: ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"]
    },
    JP: {
        archives: "ã‚¢ãƒ¼ã‚«ã‚¤ãƒ–",
        localImport: "ãƒ­ãƒ¼ã‚«ãƒ«ã‚¤ãƒ³ãƒãƒ¼ãƒˆ",
        backArchives: "â† ã‚¢ãƒ¼ã‚«ã‚¤ãƒ–ã«æˆ»ã‚‹",
        dragFiles: "ã“ã“ã«ãƒ•ã‚¡ã‚¤ãƒ«ã‚’ãƒ‰ãƒ©ãƒƒã‚°",
        browse: "å‚ç…§",
        tracer: "ãƒˆãƒ¬ãƒ¼ã‚µãƒ¼",
        simulationDay: "ã‚·ãƒŸãƒ¥ãƒ¬ãƒ¼ã‚·ãƒ§ãƒ³æ—¥æ•°",
        switchTracer: "ãƒˆãƒ¬ãƒ¼ã‚µãƒ¼åˆ‡æ›¿ (æ—¥æœ¬ â†” ãƒ–ãƒ©ã‚¸ãƒ«)",
        seasonSummer: "å­£ç¯€ï¼šå¤",
        seasonWinter: "å­£ç¯€ï¼šå†¬",
        modeVapor: "ãƒ¢ãƒ¼ãƒ‰ï¼šæ°´è’¸æ°—ã®ã¿",
        modeRain: "ãƒ¢ãƒ¼ãƒ‰ï¼šæ°´è’¸æ°— + é›²",
        windOn: "é¢¨çŸ¢ï¼šON",
        windOff: "é¢¨çŸ¢ï¼šOFF",
        view3D: "è¡¨ç¤ºï¼š3Dåœ°çƒå„€",
        view2D: "è¡¨ç¤ºï¼š2Dåœ°å›³",
        viewSplit: "è¡¨ç¤ºï¼šæ¯”è¼ƒãƒ“ãƒ¥ãƒ¼",
        play: "â–¶ å†ç”Ÿ",
        pause: "â¸ ä¸€æ™‚åœæ­¢",
        speed025: "é€Ÿåº¦ï¼š0.25x",
        speed05: "é€Ÿåº¦ï¼š0.5x",
        speed1: "é€Ÿåº¦ï¼š1x",
        headerData: "ãƒ‡ãƒ¼ã‚¿ãƒ¬ã‚¤ãƒ¤ãƒ¼",
        headerView: "ã‚«ãƒ¡ãƒ©è¡¨ç¤º",
        headerPlayback: "å†ç”Ÿ",
        alertLocalSeason: "ãƒ­ãƒ¼ã‚«ãƒ«ã‚¤ãƒ³ãƒãƒ¼ãƒˆãƒ¢ãƒ¼ãƒ‰ã§å­£ç¯€ã‚’å¤‰æ›´ã™ã‚‹ã«ã¯ã€å¯¾å¿œã™ã‚‹æ–°ã—ã„.ftãƒ•ã‚¡ã‚¤ãƒ«ã‚’ãƒ‰ãƒ©ãƒƒã‚°ã—ã¦ãã ã•ã„ã€‚",
        legendVapor: "æ°´è’¸æ°—ãƒˆãƒ¬ãƒ¼ã‚µãƒ¼ [kg/mÂ²]",
        legendRain: "ãƒˆãƒ¬ãƒ¼ã‚µãƒ¼é™æ°´é‡ > 0.1 mm/day",
...
const SEASONS = [
    {
        prefix: 'g_cma202307_09',
        label: 'Summer 2023 â€” JP tracer (00Z01JUL2023)',
        tdefStart: new Date('2023-07-01T00:00:00Z'),
        increment: 86400000
    },
    {
        prefix: 'g_cma202301_03',
        label: 'Winter 2023 â€” JP tracer (00Z01JAN2023)',
        tdefStart: new Date('2023-01-01T00:00:00Z'),
        increment: 86400000
    }
];
...
const N_PARTICLES = 3500;   // Couverture globale trÃ¨s dense
const TRAIL_LEN = 35;       // CORRECTION : Traits beaucoup plus longs (anciennement 12)
const WIND_RADIUS = 1.002;  // TrÃ¨s proche de la surface
...
function updateTopWinds(frameIdx) {
    ...
    // On divise le monde en 8 zones (2 colonnes x 4 rangÃ©es)
    const zones = Array.from({ length: 8 }, () => []);

    for (let r = 0; r < 94; r++) {
        const lat = GAUSSIAN_LATS[r];
        const zoneR = Math.floor(r / (94 / 4)); // 0 Ã  3

        for (let c = 0; c < 192; c++) {
            const lon = (c / 192) * 360;
            const zoneC = Math.floor(c / (192 / 2)); // 0 Ã  1
            const zoneIdx = zoneR * 2 + zoneC;

            const idx = base + r * 192 + c;

            // 1. On rÃ©cupÃ¨re la valeur du traceur (PWAT) Ã  cet endroit exact
    ...
    currentTopWinds = [];
    const perZone = Math.ceil(N_PARTICLES / 8); // On veut environ 3-4 flÃ¨ches par zone
    ...
    zones.forEach(z => {
        ...
        for (const pt of z) { // On boucle sur 'z' (la zone) et pas 'windSpeeds'
            ...
            for (const s of currentTopWinds) {
                // EXCLUSION RADICALE (25Â° Lat, 50Â° Lon) pour forcer la distribution sur tout le globe
...
function updateLanguageUI() {
    ...
    if (btnAuto) {
        btnAuto.innerText = autoRotateEnabled ?
            (currentLang === 'EN' ? "Auto-Rotate: ON" : "è‡ªå‹•å›žè»¢ï¼šON") :
            (currentLang === 'EN' ? "Auto-Rotate: OFF" : "è‡ªå‹•å›žè»¢ï¼šOFF");
    }
    if (btnPlay) {
    ...
    if (btnSpeed) {
        ...
        btnSpeed.innerText = t[speedKeys[playbackSpeedIndex]].replace('Speed: ', '').replace('é€Ÿåº¦ï¼š', '');
    }
...
function resize2DCanvas() {
    ...
    // Le canvas HTML 2D (overlay vent + cÃ´tes) occupe tout le conteneur
    ...
    // CAMÃ‰RA ORTHOGRAPHIQUE : cadre EXACTEMENT le plan 2.04 x 1.0
    // On ajuste le frustum pour que le plan soit toujours centrÃ© et non rognÃ©,
    // quelle que soit la forme de l'Ã©cran (letterbox / pillarbox).
    const screenAspect = rect.width / rect.height;
    const planeAspect  = 2.04 / 1.0;

    let camHalfW, camHalfH;
    if (screenAspect >= planeAspect) {
        // Ã‰cran plus large que la carte â†’ pillarbox : la hauteur dÃ©termine tout
        camHalfH = 0.5;                        // plan 1.0 de haut
        camHalfW = camHalfH * screenAspect;    // plus large que 1.02
    } else {
        // Ã‰cran plus Ã©troit que la carte â†’ letterbox : la largeur dÃ©termine tout
        camHalfW = 1.02;                       // plan 2.04 de large
        camHalfH = camHalfW / screenAspect;    // plus haut que 0.5
    }

    camera2D.left   = -camHalfW;
    camera2D.right  =  camHalfW;
...
function updateCameras() {
    // ðŸ›¡ï¸ BOUCLIER 1 : Force un minimum de 1 pixel pour Ã©viter le bug NaN (Ã©cran noir)
...
let isPlaying = false;
let lastFrameTime = 0;
let lastWindTime = 0; // ChronomÃ¨tre dÃ©diÃ© Ã  l'animation des particules
let currentFPS = 24; // Vitesse bloquÃ©e en mode 0.5x
...
// Fond bleu ocÃ©an : Ã©limine les barres noires quand l'Ã©cran est plus large que 2.04:1
...
// On initialise Ã  0 pour que (now - 0 > 10000) soit vrai dÃ¨s le chargement (Auto-start immÃ©diat)
let lastInteractionTime = 0;
let isDraggingGlobe = false;
const INACTIVITY_DELAY = 10000;

// On Ã©coute uniquement les Ã©vÃ©nements des OrbitControls (le mouvement du globe)
...
controls.addEventListener('end', () => {
    ...
    lastInteractionTime = performance.now(); // Le dÃ©compte des 10s commence quand on lÃ¢che le globe
});
// ----------------------------
...
uniform float u_offsetX; // FIX BORD 2D : dÃ©calage UV pour scrolling infini
varying vec2 vUv;
...
void main() {
    ...
    if (u_is3D > 0.5) {
        ...
        // FIX BORD 2D : u_offsetX dÃ©file la texture sans fin grÃ¢ce au RepeatWrapping
    ...
    // ETAT 2 : Mode Rain classique (SphÃ¨re de donnÃ©es 100% transparente)
    else if (u_mode > 0.5) {
        alpha = 0.0; // Laisse apparaÃ®tre le globe matÃ©riel de base
    } 
...
const dataPlaneGeom = new THREE.PlaneGeometry(2.04, 1.0); // Ratio 192/94 â€” plan 1x, RepeatWrapping gÃ¨re le bord
...
// basePlane trÃ¨s large (100 unitÃ©s) pour remplir n'importe quel ratio d'Ã©cran sans bandes noires
...
const CITIES_DB = [
    ...
    { name: "Rome", lat: 41.9028, lon: 12.4964, cc: "IT" },
    ...
    { name: "Tehran", lat: 35.6892, lon: 51.3890, cc: "IR" },
    ...
    { name: "Tashkent", lat: 41.2995, lon: 69.2401, cc: "UZ" },
    { name: "Almaty", lat: 43.2220, lon: 76.8512, cc: "KZ" },
    { name: "Ulaanbaatar", lat: 47.9200, lon: 106.9200, cc: "MN" },
    { name: "Taipei", lat: 25.0330, lon: 121.5654, cc: "TW" },
    { name: "Dhaka", lat: 23.8103, lon: 90.4125, cc: "BD" },
    { name: "Karachi", lat: 24.8607, lon: 67.0011, cc: "PK" },
    { name: "Vladivostok", lat: 43.1198, lon: 131.8869, cc: "RU" },
    { name: "Novosibirsk", lat: 55.0084, lon: 82.9357, cc: "RU" },
    { name: "Yakutsk", lat: 62.0355, lon: 129.6755, cc: "RU" }, // L'une des villes les plus froides

    // --- OCÃ‰ANIE & PACIFIQUE ---
    { name: "Sydney", lat: -33.8688, lon: 151.2093, cc: "AU" },
    { name: "Melbourne", lat: -37.8136, lon: 144.9631, cc: "AU" },
    { name: "Perth", lat: -31.9505, lon: 115.8605, cc: "AU" },
    { name: "Darwin", lat: -12.4634, lon: 130.8456, cc: "AU" },
    { name: "Auckland", lat: -36.8485, lon: 174.7633, cc: "NZ" },
    { name: "Wellington", lat: -41.2865, lon: 174.7762, cc: "NZ" },
    { name: "Port Moresby", lat: -9.4431, lon: 147.1803, cc: "PG" },
    { name: "Suva", lat: -18.1248, lon: 178.4501, cc: "FJ" }, // Fidji
    { name: "Noumea", lat: -22.2758, lon: 166.4580, cc: "NC" }, // Nouvelle-CalÃ©donie
    { name: "Papeete", lat: -17.5334, lon: -149.5667, cc: "PF" } // Tahiti (parfait pour l'ocÃ©an Pacifique)
];
...
function createMarker(lat, lon, labelText, countryCode, isPrimary = false, r = 1.11) {
    ...
    // Taille du point rÃ©duite pour les villes secondaires
    ...
    // Canvas haute rÃ©solution Ã©largi pour les noms complets avec drapeaux
    ...
    sprite.scale.set(0.24, 0.06, 1); // Ratio 4:1 adaptÃ© au nouveau canvas 512x128
    ...
    for (let i = 0; i < 5; i++) {
        ...
        slot.innerHTML = `
            <div class="slot-header" id="slot-header-${i}" style="opacity: 0.3;">
                <img id="flag-${i}" src="" width="16" style="display:none; border-radius: 2px;">
                <span class="slot-select" id="city-name-${i}">En attente...</span>
            </div>
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (slot) {
            if (i === currentSlotIndex) {
                ...
                slot.style.borderLeft = '1px solid rgba(255, 255, 255, 0.1)';
...
function updateViseur() {
    ...
    const city = CITIES_DB[hoveredCityIndex];
    ...
    // Calcul pour faire voler la camÃ©ra (Spherical Coords)
    const targetPhi = Math.max(0.1, Math.min(Math.PI - 0.1, (90 - city.lat) * (Math.PI / 180)));
    const targetTheta = (city.lon + 90) * (Math.PI / 180);
...
function confirmCitySelection() {
    ...
    selectedSlots[currentSlotIndex] = hoveredCityIndex;

    // UI update
    const header = document.getElementById(`slot-header-${currentSlotIndex}`);
    const nameEl = document.getElementById(`city-name-${currentSlotIndex}`);
    const flagEl = document.getElementById(`flag-${currentSlotIndex}`);
    if (header) header.style.opacity = '1';
    if (nameEl) nameEl.innerText = CITIES_DB[hoveredCityIndex].name;
    if (flagEl) {
        flagEl.src = `https://flagcdn.com/w20/${CITIES_DB[hoveredCityIndex].cc.toLowerCase()}.png`;
...
function startArcadeSimulation() {
    gameState = 'SIMULATE';
    document.getElementById('arcade-viseur').style.display = 'none';
...
function resetArcadeSimulation() {
    console.log("âš ï¸ RESET ARCADE : Retour au mode sÃ©lection");

    // 1. Changer l'Ã©tat global
    gameState = 'SELECT';
    currentSlotIndex = 0;
    selectedSlots = [-1, -1, -1, -1, -1];

    // 2. FORCER l'arrÃªt du temps (IndÃ©pendant du bouton)
    isPlaying = false;

    // 3. Remettre le bouton Play/Pause en mode "PrÃªt Ã  lire"
    const playBtn = document.getElementById('btn-play');
    if (playBtn) {
        const t = TRANSLATIONS[currentLang];
        // On remet le texte "PLAY" (ou l'icÃ´ne correspondante dans ton code)
        playBtn.innerText = t.play || "PLAY";
        playBtn.style.background = '#2a2a2a'; // Couleur grise par dÃ©faut
    }

    // 4. Vider toutes les donnÃ©es accumulÃ©es des villes
    ...
    // 5. RÃ©initialiser la chronologie (Rembobinage au Jour 1)
    PARAMS.currentFrame = 0;
    const sliderTime = document.getElementById('slider-time');
    if (sliderTime) {
        sliderTime.value = 0;
        // On met Ã  jour la barre visuelle si elle existe
        const trackFill = document.querySelector('.slider-track-fill');
        if (trackFill) trackFill.style.width = '0%';
    }

    // 6. RÃ©afficher le viseur et reconstruire les slots vides
    ...
    // 7. RafraÃ®chir l'affichage global (pour vider les chiffres en bas)
    updateFrame();
}
...
async function loadData() {
    ...
    const fetchBuf = async (url) => {
        try {
            ...
            // SÃ©curitÃ© absolue : on ignore les pages d'erreur HTML cachÃ©es en 404
            if (arrayBuf.byteLength < 1000000) return null;
            return expandBuffer(new Float32Array(arrayBuf), 91, 4);
        } catch (e) {
            console.error("Erreur de tÃ©lÃ©chargement :", url);
            return null;
        }
    };

    if (!isLocalData) {
        // 1. On dÃ©truit les anciens vents pour Ã©viter l'effet fantÃ´me de l'Ã©tÃ©
        buffer1 = null; archiveBufferPrecip = null;
        archiveBufferU = null; archiveBufferV = null;

        // 2. Chargement parallÃ¨le (beaucoup plus rapide)
        ...
        // (Math.min est une sÃ©curitÃ© si on charge un dataset plus court)
        PARAMS.currentFrame = Math.min(PARAMS.currentFrame, PARAMS.frames - 1);

        if (sliderTime) {
            sliderTime.max = PARAMS.frames - 1;
            sliderTime.value = PARAMS.currentFrame; // Le slider reste Ã  sa place
        }
...
function updateFrame() {
    ...
    const precalcIndices = selectedSlots.map(cityIdx => {
        if (cityIdx === -1) return 0; // SÃ©curitÃ© Arcade
    ...
    for (let f = 0; f <= PARAMS.currentFrame; f++) {
        ...
        selectedSlots.forEach((cityIdx, slotIdx) => {
            if (cityIdx === -1) return; // SÃ©curitÃ© Arcade
    ...
    // Mise Ã  jour UI et Animations des slots
    selectedSlots.forEach((cityIdx, slotIdx) => {
        if (cityIdx === -1) return; // SÃ©curitÃ© Arcade
        ...
        if (valEl) {
            valEl.innerText = (PARAMS.displayMode === 'pwat')
                ? slotData[slotIdx].totalVapor.toFixed(1) + " kg/mÂ²"
                : `${slotData[slotIdx].totalVapor.toFixed(1)} kg/mÂ² | ${slotData[slotIdx].totalWater.toFixed(1)} L/mÂ²`;
        }
        ...
        if (dayEl) {
            if (slotData[slotIdx].firstTouchDay !== -1) {
                ...
                if (slotData[slotIdx].isPingFrame) {
                    ...
                    // Joue le son de ping si tu l'as ajoutÃ© dans l'HTML
                    const pingSfx = document.getElementById('sfx-ping');
                    if (pingSfx) {
                        pingSfx.currentTime = 0;
                        pingSfx.play().catch(e => console.log("Audio play blocked by browser policy"));
                    }
                    setTimeout(() => {
                        if (dayEl) {
                            dayEl.style.color = '#7ecfff';
    ...
    if (aU && aV && windTexture) {
        for (let i = 0; i < 192 * 94; i++) {
            ...
            windTexture.image.data[i * 4 + 1] = Math.max(0, Math.min(1, normV));
            windTexture.image.data[i * 4 + 2] = 0;
            windTexture.image.data[i * 4 + 3] = 1;
        }
        windTexture.needsUpdate = true;
    }
...
function spawnParticle(i, keepPosition = false) {
    ...
    if (!keepPosition) {
        // Nouvelle naissance alÃ©atoire (quand la particule meurt de vieillesse)
        startLat = Math.asin(Math.random() * 2 - 1) * (180 / Math.PI);
        startLon = Math.random() * 360;

        // CORRECTION : DurÃ©e de vie doublÃ©e pour supporter les longues traÃ®nÃ©es
        ...
        // SÃ©curitÃ© si la particule est vide
        if (tailX === 0 && tailY === 0 && tailZ === 0) {
            startLat = Math.asin(Math.random() * 2 - 1) * (180 / Math.PI);
            startLon = Math.random() * 360;
        } else {
            // Extraction mathÃ©matique des coordonnÃ©es Lat/Lon depuis la 3D
    ...
    // PrÃ©-calcul de la forme de la traÃ®nÃ©e avec la donnÃ©e du vent actuel
    ...
    // Mise Ã  jour de la tÃªte de lecture pour l'animation continue
    pLat[i] = cLat;
    pLon[i] = cLon;
}
...
function updateParticles(frameIdx, doAdvance) {
    ...
    // âš ï¸ On a supprimÃ© le verrou de pause. Les vents soufflent dÃ©sormais en permanence Ã  60 FPS, 
    // mÃªme quand la simulation (les jours) est en pause. C'est le comportement officiel de Windy.com.
...
function alignParticlesToFrame(frameIdx) {
    ...
    for (let i = 0; i < N_PARTICLES; i++) {
        ...
        // On reconstruit la traÃ®nÃ©e Ã  l'envers (de la tÃªte vers la queue)
        for (let t = TRAIL_LEN - 1; t >= 0; t--) {
            ...
            // On "recule" dans l'espace pour aligner le point prÃ©cÃ©dent sur le nouveau vent
...
function render2D() {
    ...
    // â”€â”€ CALCUL DE LA ZONE DU PLAN 2.04Ã—1.0 dans le canvas HTML â”€â”€
    // La camÃ©ra orthographique (left=-camHalfW, right=+camHalfW, top=+camHalfH, bottom=-camHalfH)
    // projette le plan 2.04Ã—1.0 sur une portion du canvas.
    // On recalcule ici les bornes en pixels pour clipper le dessin.
    const rect = canvas2D; // dimensions dÃ©jÃ  en pixels physiques
    const screenW = W / dpr;
    const screenH = H / dpr;
    const screenAspect = screenW / screenH;
    const planeAspect  = 2.04;

    let planePixW, planePixH, planeOffX, planeOffY;
    if (screenAspect >= planeAspect) {
        // Ã‰cran plus large : hauteur contrainte, la largeur du plan < largeur de l'Ã©cran
        planePixH = screenH;
        planePixW = screenH * planeAspect;
        planeOffX = (screenW - planePixW) / 2;
        planeOffY = 0;
    } else {
        // Ã‰cran plus Ã©troit : largeur contrainte, la hauteur du plan < hauteur de l'Ã©cran
        planePixW = screenW;
    ...
    // DESSIN DYNAMIQUE DES VILLES SÃ‰LECTIONNÃ‰ES
    ...
    if (PARAMS.showWind && aU2D && aV2D) {
        ...
        for (let i = 0; i < max2DParticles; i++) {
            ...
            // FIX FOUET : prevX initialisÃ© Ã  null (pas -1) pour la dÃ©tection correcte du premier point
            let prevX = null;
            let prevY = null;
            let started = false;
            ...
            for (let t = 0; t < max2DTrail; t++) {
                ...
                if (!started) {
                    ...
                    // FIX FOUET : Si le saut en X est > 50% de la largeur, c'est une tÃ©lÃ©portation
                ...
                // FIX FOUET : Normaliser la longitude pour garder la continuitÃ© de projection
                cLon = ((cLon % 360) + 360) % 360;
            }
...
sliderTime.addEventListener('input', (e) => {
    ...
    // NOUVEAU : RÃ©alignement instantanÃ© de la forme des vents sur la nouvelle frame
    alignParticlesToFrame(PARAMS.currentFrame);

    updateFrame();
});

let currentSeason = 'summer'; // Nouvelles variables d'Ã©tat (saison)
if (btnToggle) {
    btnToggle.innerText = TRANSLATIONS[currentLang].seasonSummer;

    // Remplacement de l'Ã©vÃ©nement de clic
    btnToggle.addEventListener('click', () => {
        ...
        // Mise Ã  jour du label en haut Ã  gauche
        if (typeof uiLabelSet !== 'undefined' && uiLabelSet) {
            uiLabelSet.innerText = `PWAT1 (Japan) â€” ${currentSeason === 'summer' ? 'Summer' : 'Winter'}`;
        }

        // Action de chargement des donnÃ©es
...
btnToggleWind.addEventListener('click', () => {
    ...
    updateFrame(); // Force le rafraÃ®chissement
});

const btnToggleDataType = document.getElementById('btn-toggle-data-type');

// Couleurs de la vapeur (de trÃ¨s transparent Ã  blanc pur opaque)
const VAPOR_COLORS = [
...
function applyLegendColors(colors) {
    ...
    colors.forEach((c, i) => {
        if (blocks[i]) {
            ...
            // Gestion intelligente de la lisibilitÃ© du texte "Tr" et ">10"
            if (i === 0) {
...
if (btnToggleDataType) {
    btnToggleDataType.addEventListener('click', () => {
        ...
        if (PARAMS.displayMode === 'pwat') {
            ...
            // VisibilitÃ© des nuages activÃ©e par dÃ©faut en mode precip (Vapor + Clouds)
...
if (btnToggleAutoRotate) {
    ...
    btnToggleAutoRotate.addEventListener('click', () => {
        ...
        // Si on dÃ©sactive manuellement, on force l'arrÃªt immÃ©diat
...
function createAtmosphere() {
    ...
    // Ã‰clairage global doux (HemisphereLight)
    ...
    function createAtmosphereMaterial(is3D) {
        const mat = new THREE.MeshStandardMaterial({
            ...
            bumpMap: is3D ? cloudColorTex : cloudColorTex2D,  // NOUVEAU : CrÃ©e le relief 3D avec la lumiÃ¨re
            bumpScale: 0.025,                                 // NOUVEAU : IntensitÃ© du relief
            transparent: true,
            opacity: 1.0,
            depthWrite: false,
            roughness: 0.7,
            metalness: 0.0,
            side: THREE.DoubleSide // Permet Ã  la lumiÃ¨re interne de l'Ã©clair de fonctionner
        });
...
vec4 textureBilinear(sampler2D tex, vec2 uv, vec2 texSize) {
    ...
    // --- Version 3D Sphere (VolumÃ©trique) ---
    ...
    for (let i = 0; i < SHELL_COUNT; i++) {
        const scale = 1.0 + shellHeights[i] * (0.06 / 1.05); // Ã‰paisseur 3D du nuage = 0.06 unitÃ©s
    ...
    // --- Version 2D Plane (VolumÃ©trique) ---
...
initParticles(); // DÃ©marrage du systÃ¨me de particules
createAtmosphere();
...
// --- GESTION CORRIGÃ‰E DE LA PÃ‰DALE (ARCADE) ---

window.addEventListener('keydown', (e) => {
    if (e.key.toLowerCase() === 'b' && !e.repeat) {
        pedalPressTime = performance.now();

        // On lance le chrono de reset peu importe l'Ã©tat
        // ModifiÃ© : reset aprÃ¨s 3 secondes (3000 ms)
        pedalHoldTimer = setTimeout(() => {
            resetArcadeSimulation();
            console.log("SystÃ¨me rÃ©initialisÃ© aprÃ¨s 3 secondes.");
        }, 3000);
    }
});
...
window.addEventListener('keyup', (e) => {
    if (e.key.toLowerCase() === 'b') {
        // 1. On arrÃªte immÃ©diatement le chrono de reset
        clearTimeout(pedalHoldTimer);

        // 2. On calcul la durÃ©e rÃ©elle de l'appui
        let pressDuration = performance.now() - pedalPressTime;

        // 3. LOGIQUE DE SÃ‰CURITÃ‰ : 
        // ModifiÃ© : on n'exÃ©cute l'action classique que si l'appui dure moins de 2.8 secondes (2800 ms)
        if (pressDuration < 2800) {
            if (gameState === 'SELECT') {
                confirmCitySelection(); // Lock la ville uniquement sur appui court
            } else if (gameState === 'SIMULATE') {
                const playBtn = document.getElementById('btn-play');
                if (playBtn) playBtn.click(); // Play/Pause uniquement sur appui court
            }
        }
    }
});
...
function animateLoop(t) {
    ...
    // â”€â”€â”€ PHASE 2 : SIMULATION (ContrÃ´le Manuel au Joystick) â”€â”€â”€
    if (gameState === 'SIMULATE') {
        if (serialPort && PARAMS.viewMode === 0) {
            ...
            if (Math.abs(rawX) > deadzone || Math.abs(rawY) > deadzone) {
                ...
                let movePhi = rawX > deadzone ? rawX - deadzone : (rawX < -deadzone ? rawX + deadzone : 0);

                spherical.theta += moveTheta * speed;
                spherical.phi -= movePhi * speed;
    ...
    // â”€â”€â”€ PHASE 1 : SÃ‰LECTION (Pilote Automatique via PotentiomÃ¨tre) â”€â”€â”€
    else if (gameState === 'SELECT' && targetCameraSpherical) {
        controls.autoRotate = false; // Coupe la rotation auto pendant qu'on choisit

        let currentSph = new THREE.Spherical().setFromVector3(camera3D.position);

        // L'effet "Ã©lastique" (Lerp) : glisse de 5% (0.05) vers la cible
        currentSph.phi += (targetCameraSpherical.phi - currentSph.phi) * 0.05;

        // Calcul du chemin le plus court pour ne pas faire un tour complet sur soi-mÃªme
        let dTheta = targetCameraSpherical.theta - currentSph.theta;
        if (dTheta > Math.PI) dTheta -= Math.PI * 2;
        if (dTheta < -Math.PI) dTheta += Math.PI * 2;
        currentSph.theta += dTheta * 0.05;
    ...
    if (isPlaying && (t - lastFrameTime >= msPerFrame)) {
        // VERROU STRICT : Aucun rattrapage mathÃ©matique. 
        // Si le GPU rame (ex: vue comparative), la simulation attend son tour et ralentit gracieusement, 
        // ce qui Ã©vite les sauts de jours et les accÃ©lÃ©rations brutales.
        lastFrameTime = t;
        ...
        if (PARAMS.currentFrame === 0) {
            ...
            // Alternance 2D/3D automatique a la fin de la simulation
            PARAMS.viewMode = (PARAMS.viewMode === 0) ? 1 : 0;
            const canvas2DContainer = document.getElementById('canvas-2d-container');
            if (canvas2DContainer) {
                if (PARAMS.viewMode === 1) {
                    canvas2DContainer.style.display = 'flex';
            ...
            if (typeof atmosphereSphere !== 'undefined' && atmosphereSphere) {
                atmosphereSphere.visible = (PARAMS.displayMode === 'precip' && PARAMS.viewMode === 0);
            }
            if (typeof atmospherePlane !== 'undefined' && atmospherePlane) {
                atmospherePlane.visible = (PARAMS.displayMode === 'precip' && PARAMS.viewMode === 1);
            }
        }
        updateFrame();
        _doAdvance = true;
    ...
    // RÃ‰GULATEUR DE CHARGE CPU : On bride l'animation du vent Ã  30 FPS max (toutes les ~33ms) ou 20 FPS (50ms) en Precip
    // Cela libÃ¨re Ã©normÃ©ment de puissance processeur pour que le mode Comparative reste fluide.
    const windDelay = (PARAMS.displayMode === 'precip') ? 50 : 33;
    if (t - lastWindTime > windDelay) {
        updateParticles(PARAMS.currentFrame, _doAdvance);
        if (PARAMS.viewMode !== 0) render2D(); // NOUVEAU : La 2D est dessinÃ©e en flux continu
        lastWindTime = t;
    }
    ...
    if (PARAMS.viewMode === 0) {
        ...
        if (coastMesh) coastMesh.visible = true; // ðŸ›¡ï¸ SÃ‰CURITÃ‰ ICI
        ...
        markers.forEach((m, idx) => {
            ...
            // Visible SI sÃ©lectionnÃ©e ET du bon cÃ´tÃ© du globe
        ...
        if (coastMesh) coastMesh.visible = false; // ðŸ›¡ï¸ SÃ‰CURITÃ‰ ICI
        if (atmosphereSphere) atmosphereSphere.visible = false;
        ...
        // ðŸ›¡ï¸ ON CACHE LES FLÃˆCHES 3D ICI
        ...
        // ðŸ›¡ï¸ ON RÃ‰ACTIVE LES FLÃˆCHES 3D ICI
...
// ============================================================================
// â”€â”€ UI V3 : DRAG & DROP ET LECTURE MULTIPLE (.bin / .nc) â”€â”€
// ============================================================================
const tabArchives = document.getElementById('tab-archives');
const tabUpload = document.getElementById('tab-upload');
...
// 1. Activation de la sÃ©lection multiple
if (fileInput) {
    fileInput.setAttribute('multiple', '');
    fileInput.setAttribute('accept', '.nc,.bin,.ft*');
}

// -- VÃ‰RITABLE DRAG & DROP --
if (dropZoneBox) {
...
// --- LOGIQUE DES ONGLETS CORRIGÃ‰E ---
// --- LOGIQUE DES ONGLETS CORRIGÃ‰E ET COMPLÃˆTE ---
if (tabArchives && tabUpload) {
    ...
    tabArchives.addEventListener('click', () => {
        ...
        // 5. On recharge les donnÃ©es d'archives
        isLocalData = false;
        PARAMS.currentFrame = 0;
        loadData();
    });
    ...
    tabUpload.addEventListener('click', () => {
        ...
        // 4. Ã‰tire le conteneur et centre le texte au milieu de l'Ã©cran si pas de donnÃ©es locales
        if (uiDateDisplay) {
            if (!localBuffer) {
                ...
                uiDateDisplay.parentElement.style.width = "auto";
                uiDateDisplay.parentElement.style.textAlign = "left";
                // L'affichage de la banniÃ¨re se mettra Ã  jour automatiquement via updateFrame()
            }
        }

        // 5. DÃ©clenche la fonction qui met le globe Ã  zÃ©ro
        isLocalData = true;
        updateFrame();
    });
...
async function scanFileForVariables(file) {
    ...
    try {
        ...
        if (result.variables && result.variables.length > 0) {
            ...
            // On lance le dÃ©codage initial avec la premiÃ¨re variable de la liste
...
function buildVariableMenu(variables) {
    ...
    // Si le menu n'existe pas, on le crÃ©e
    if (!selectMenu) {
        ...
        // Ã‰vÃ©nement : Relance le dÃ©codage complet si on change de variable
...
async function processMultipleGRIBWithVercel(files, paramId = 150) {
    try {
        console.warn(`[DEBUG IsoGSM] processMultipleGRIBWithVercel appelÃ© avec ${files.length} fichiers !`);
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                const [data150, dataP, dataU, dataV] = await Promise.all([
                    ...
                    fetchParam(67, files[i]),  // PRATE1 (Pluie tracÃ©e)
        ...
        console.warn(`[DEBUG IsoGSM] FIN. Mise Ã  jour de PARAMS.frames Ã  ${framesLoaded} !`);
...
async function readMultipleBinFiles(files) {
    ...
    // ---------------------------------------------------------
    // ðŸ› ï¸ RÃ‰GLAGE IMPORTANT : Index de la variable (Record)
    // Ton fichier de 2.6 Mo contient plein de variables.
    // PWAT n'est probablement pas la premiÃ¨re (0). 
    ...
    try {
        for (let file of files) {
            ...
            // 1. DÃ©tection automatique de l'Endianness
...
// 2. Mise Ã  jour du HUD dans updateFrame
...
function initTablets() {
    ...
    const season = urlParams.get('season') === 'winter' ? 'winter' : 'summer';
    setTimeout(() => {
        if (season === 'winter') {
            const btnToggle = document.getElementById('toggle-data');
