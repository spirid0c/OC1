...
let gameState = 'SELECT'; // 'SELECT' (Préparation) ou 'SIMULATE' (Action)
...
let targetCameraSpherical = null; // Pour faire voler la caméra
...
const CITIES_DB = [
    ...
    { name: "Manaus", lat: -3.1190, lon: -60.0217, cc: "BR" }, // Plein cœur de l'Amazonie

    { name: "Buenos Aires", lat: -34.6037, lon: -58.3816, cc: "AR" },

    { name: "Ushuaia", lat: -54.8019, lon: -68.3030, cc: "AR" }, // Extrême Sud
    ...
    { name: "Noumea", lat: -22.2758, lon: 166.4580, cc: "NC" }, // Nouvelle-Calédonie
...
function updateViseur() {
    ...
    // Calcul pour faire voler la caméra (Spherical Coords)
...
function resetArcadeSimulation() {
    ...
    // 4. Vider toutes les données accumulées des villes
    ...
    // 5. Réinitialiser la chronologie (Rembobinage au Jour 1)
...
function updateFrame() {
    ...
    const precalcIndices = selectedSlots.map(cityIdx => {
        ...
        if (cityIdx === -1) return 0; // Sécurité Arcade
        ...
        if (dayEl) {
            ...
            if (slotData[slotIdx].firstTouchDay !== -1) {
                ...
                if (slotData[slotIdx].isPingFrame) {
                    ...
                    // Joue le son de ping si tu l'as ajouté dans l'HTML
...
// --- GESTION CORRIGÉE DE LA PÉDALE (ARCADE) ---
...
window.addEventListener('keydown', (e) => {
    ...
    if (e.key.toLowerCase() === 'b' && !e.repeat) {
        ...
        // Modifié : reset après 3 secondes (3000 ms)
...
window.addEventListener('keyup', (e) => {
    ...
    if (e.key.toLowerCase() === 'b') {
        ...
        // Modifié : on n'exécute l'action classique que si l'appui dure moins de 2.8 secondes (2800 ms)
...
function animateLoop(t) {
    ...
    // ─── PHASE 1 : SÉLECTION (Pilote Automatique via Potentiomètre) ───
